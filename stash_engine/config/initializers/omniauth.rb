Rails.application.config.middleware.use OmniAuth::Builder do
  ...
  OmniAuth.config.full_host = ->(env) do
    ...
    Rails.logger.debug("HTTP_X_FORWARDED_HOST = #{env['HTTP_X_FORWARDED_HOST']}")
    Rails.logger.debug("HTTP_HOST = #{env['HTTP_HOST']}")
    ...
    Rails.logger.debug("output: #{u.scheme}://#{u.host}#{(u.port == 80 ? '' : ":#{u.port}")}")
