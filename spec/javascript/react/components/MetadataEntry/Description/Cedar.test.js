...
describe('Cedar', () => {
  let resource; let templates; let templateName;
  ...
  beforeEach(() => {
    ...
    templates = [{id: 1, title: 'Empty template'}, {id: 2, title: templateName}];
  });

  it('renders a select box', () => {
    render(<Cedar resource={resource} setResource={setResource} templates={templates} />);
    expect(screen.queryByText('Choose a metadata form')).toBeInTheDocument();

    const options = screen.getAllByRole('option');
    expect(options.length).toBe(3);
    const optionNames = options.map((o) => o.label);
    expect(optionNames).toEqual(['Select a relevant form', 'Empty template', templateName]);
  });

  it('renders an indicator that metadata is present', () => {
    resource.cedar_json = {template_id: 2, json: {metadata: 2}};
    render(<Cedar resource={resource} setResource={setResource} templates={templates} />);
  ...
  it('opens the editor modal', async () => {
    resource.cedar_json = {template_id: 1, json: {metadata: 2}};
    render(<Cedar resource={resource} setResource={setResource} templates={templates} />);
