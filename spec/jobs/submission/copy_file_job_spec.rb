...
RSpec.describe Submission::CopyFileJob, type: :job do
  ...
  before do
    ...
    allow(StashEngine::NotificationsMailer).to receive_message_chain(:error_report, :deliver_now)
  ...
  describe '#perform' do
    ...
    context 'when copy takes too long (timeout)' do
      ...
      it 'creates an errored repo_queue_state and sends an email' do
        ...
        expect(StashEngine::NotificationsMailer).to have_received(:error_report).with(resource, instance_of(StandardError))
      end
    end
  end
