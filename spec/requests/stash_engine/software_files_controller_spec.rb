...
module StashEngine
  RSpec.describe SoftwareFilesController, type: :request do
    ...
    describe '#presign_upload' do
      before(:each) do
        @url = Rails.application.routes.url_helpers.software_file_presign_url_path(resource_id: @resource.id)
    ...
    describe '#upload_complete' do
      ...
      before(:each) do
        @url = Rails.application.routes.url_helpers.software_file_complete_path(resource_id: @resource.id)
        @json_hash = {
    ...
    describe '#validate_urls' do
      ...
      it 'returns json when request with format html to validate urls' do
        @url = Rails.application.routes.url_helpers.software_file_validate_urls_path(resource_id: @resource.id)
    ...
    describe '#destroy_manifest' do
      ...
      it 'returns json when request with html format' do
        ...
        @url = Rails.application.routes.url_helpers.destroy_manifest_software_file_path(id: @file.id)
