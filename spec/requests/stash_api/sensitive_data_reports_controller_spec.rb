...
module StashApi
  RSpec.describe SensitiveDataReportsController, type: :request do
    ...
    describe '#show' do
      before(:each) do
        @report = create(:sensitive_data_report, generic_file: @generic_file)
        @path = Rails.application.routes.url_helpers.file_sensitive_data_report_path(@generic_file.id)
      end

      it 'shows the sensitive_data report info for a file that exists and has report (superuser)' do
        ...
        expect(hsh['status']).to eq(@report.status)
        expect(hsh['report']).to eq(@report.report)
      end
    ...
    describe '#update' do
      before(:each) do
        ...
        @report2 = create(:sensitive_data_report, generic_file: @generic_file2)
        @path = Rails.application.routes.url_helpers.file_sensitive_data_report_path(@generic_file.id)
      end

      it "adds a report that doesn't exist yet" do
        response_code = put @path,
                            params: { status: 'noissues', report: @report2.report }.to_json, # same as report2 json
                            headers: default_authenticated_headers
        expect(response_code).to eq(200)
        hsh = response_body_hash
        expect(hsh['status']).to eq('noissues')
        expect(hsh['report']).to eq(@report2.report)
      end
      ...
      it 'updates a report that does exist' do
        ...
        expect(hsh['status']).to eq('noissues')
        expect(hsh['report']).to eq(@report2.report)
        expect(StashEngine::SensitiveDataReport.find(report_id).report).to eq(@report2.report) # the id is the same
      end
      ...
      it "doesn't allow updating unless a status is correct from the list" do
        response_code = put @path,
                            params: { status: 'squid_cats', report: @report2.report }.to_json,
                            headers: default_authenticated_headers
