      ...
      describe SyncTask do
        ...
        describe '#new' do
          it 'accepts a valid "from" datestamp' do
            time = Time.new.utc
            sync_task = SyncTask.new(capability_list_uri: 'http://example.org/cap_list.xml', from_time: time)
            expect(sync_task.from_time).to eq(time)
          end

          it 'accepts a valid "until" datestamp' do
            time = Time.new.utc
            sync_task = SyncTask.new(capability_list_uri: 'http://example.org/cap_list.xml', until_time: time)
          ...
          it 'rejects datestamps that would create an invalid range' do
            ...
            expect { SyncTask.new(capability_list_uri: 'http://example.org/cap_list.xml', from_time: now, until_time: epoch) }.to raise_error(RangeError)
          end

          it 'rejects non-UTC datestamps' do
            non_utc = Time.new(2002, 10, 31, 2, 2, 2, '+02:00')
            expect { SyncTask.new(capability_list_uri: 'http://example.org/cap_list.xml', from_time: non_utc) }.to raise_error(ArgumentError)
            expect { SyncTask.new(capability_list_uri: 'http://example.org/cap_list.xml', until_time: non_utc) }.to raise_error(ArgumentError)
          end

          it 'accepts a valid capability list URL' do
            valid_url = 'http://example.org/capability_list.xml'
            task = SyncTask.new capability_list_uri: valid_url
          ...
          it 'accepts a URI object as a capability list URL' do
            ...
            task = SyncTask.new capability_list_uri: uri
            expect(task.capability_list_uri).to eq(uri)
          end

          it 'rejects an invalid capability list URL' do
            invalid_url = 'I am not a valid URL'
            expect { SyncTask.new capability_list_uri: invalid_url }.to raise_error(URI::InvalidURIError)
          end

          it 'requires a capability list URL' do
            # noinspection RubyArgCount
            expect { SyncTask.new }.to raise_error(ArgumentError)
          end
        end
