...
RSpec.feature 'Landing', type: :feature, js: true do
  ...
  describe 'landing page' do
    ...
    context 'embargoed dataset' do

      before(:each) do
        create(:curation_activity, :embargoed, resource: resource, user: curator)
        identifier.reload
      end
    ...
    context 'retracted dataset' do

      before(:each) do
        create(:curation_activity, :published, resource: resource, user: curator)
        create(:curation_activity, :retracted, resource: resource, user: curator)
        resource.reload
      end
      ...
      context 'with file_view false' do
        before(:each) do
          identifier.resources.update(file_view: false)
          identifier.resources.reload
        end

        it 'does not display the retracted README' do
          visit stash_url_helpers.landing_show_path(id: identifier.to_s)
          expect(page).to have_text('Abstract')
          expect(page).to have_text(resource.descriptions.type_abstract.first.description)
        ...
        it 'does not display the retracted files' do
          visit stash_url_helpers.landing_show_path(id: identifier.to_s)
          expect(all('details.c-file-group').count).to eq(0)
          expect(page).to have_content('This dataset has been retracted')
        end
      end
    end
  end
