  ...
  module Import
    ...
    describe Crossref do
      ...
      describe '#populate_title' do
        before(:each) do
          @cr = Crossref.new(resource: @resource, json: { 'title' => [TITLE] })
        end
        ...
        it "doesn't puke or overwrite with no title" do
          ...
          @cr = Crossref.new(resource: @resource, json: {})
          @cr.send(:populate_title)
          expect(@resource.title).to eql(@title)
        end
        ...
        it "doesn't puke or overwrite with blank title" do
          @title = 'meow meow'
          @resource.update(title: @title)
          @cr = Crossref.new(resource: @resource, json: { 'title' => [] })
      ...
      describe '#populate_authors' do
        before(:each) do
          @cr = Crossref.new(resource: @resource, json: { 'author' => AUTHOR })
        end
        ...
        it 'handles minimal data because lots of stuff is missing metadata' do
          ...
          @cr = Crossref.new(resource: @resource, json: { 'author' => @author_example })
          @cr.send(:populate_authors)
      ...
      describe '#populate_abstract' do
        before(:each) do
          @cr = Crossref.new(resource: @resource, json: { 'abstract' => ABSTRACT })
        end
      ...
      describe '#populate_subjects' do
        before(:each) do
          ...
          @cr = Crossref.new(resource: @resource, json: { 'subject' => @subj_example })
        end
      ...
      describe '#populate_funders' do
        before(:each) do
          ...
          @cr = Crossref.new(resource: @resource, json: { 'funder' => @funder_example })
        end
        ...
        it 'removes blank contributor entries before populating' do
          funders = [{ 'name' => '' }]
          funders << FUNDER
          cr = Crossref.new(resource: @resource, json: { 'funder' => funders.flatten })
          cr.send(:populate_funders)
      ...
      describe '#populate_article_type' do
        before(:each) do
          @cr = Crossref.new(resource: @resource, json: { 'URL' => URL })
        end
        ...
        it 'ignores blank URLs' do
          @cr = Crossref.new(resource: @resource, json: { 'URL' => '' })
      ...
      describe '#populate_publication_date' do
        before(:each) do
          ...
          @cr = Crossref.new(resource: @resource, json: { 'published-online' => { 'date-parts' => FUTURE_PUBLICATION_DATE } })
        end

        it 'sets the publication_date' do
          cr = Crossref.new(resource: @resource, json: { 'published-online' => { 'date-parts' => PAST_PUBLICATION_DATE } })
        ...
        it 'ignores blank published-online dates' do
          @cr = Crossref.new(resource: @resource, json: { 'published-online' => nil })
      ...
      describe '#populate_publication_name' do
        before(:each) do
          @cr = Crossref.new(resource: @resource, json: { 'publisher' => PUBLISHER })
        end
        ...
        it 'ignores blank publisher names' do
          @cr = Crossref.new(resource: @resource, json: { 'publisher' => nil })
