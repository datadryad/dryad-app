...
export default function CalculateFees({resource, fees, setFees}) {
  ...
  if (paid) {
    return (
      <>
        <p>This dataset has been previously submitted and the <a href="/requirements#cost" target="blank">Data Publishing Charge<ExitIcon /></a> has been paid for {formatSizeUnits(resource.identifier.last_invoiced_file_size)}.</p>
        {fees.storage_fee && (
  ...
  if (fees.storage_fee) {
    return (
      <p>This {formatSizeUnits(resource.total_file_size)} dataset has a <a href="/requirements#cost" target="blank">Data Publishing Charge<ExitIcon /></a> of <b>{fees.storage_fee.toLocaleString('en-US', {style: 'currency', currency: 'USD'})}</b>.</p>
    );
  }
