class AddTotalFileSize < ActiveRecord::Migration[6.1]
  def up
    ...
    StashEngine::Resource.unscoped.all.each do |r|
      # get existing file sizes
      r.update_column(:total_file_size, StashEngine::DataFile.where(resource_id: r.id).where(file_state: %w[created copied]).sum(:upload_file_size))
    end
  end
