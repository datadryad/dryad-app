...
namespace :app_data do
  ...
  task clear_data: :environment do
    ...
    response = $stdin.gets
    ClearData.clear_data if response.strip.casecmp('YES').zero? && !Rails.env.include?('production')
  end
  ...
  task clear_datasets: :environment do
    puts "Are you sure you want to clear the data in the environment #{Rails.env}?  (Type 'yes' to proceed.)"
    response = $stdin.gets
    ClearData.clear_datasets if response.strip.casecmp('YES').zero? && !Rails.env.include?('production')
  end
end
# :nocov:
