...
namespace :automatic_reports do
  ...
  task reports_by_date: :environment do
    puts ''
    puts '---------------'
    puts "Running reports on #{DateTime.current}:"
    if Date.today.day == 1
      # End of month reports
      puts "Running TATs report for year: #{1.day.ago.year}, month: #{1.day.ago.month}"
      ...
      %i[in_progress queued curation peer_review action_required].each do |status|
        puts "Running DatasetVolumeAndAge report for status: #{status}, date: #{1.day.ago}"
