...
module Stash
  class Salesforce
    class << self
      ...
      def case_email_text(case_id:)
        result = db_query("SELECT TextBody from EmailMessage Where RelatedToId = '#{case_id}'")
        result.first&.TextBody
      end
      ...
      def create_email_case(email:, subject:, body:, sname:, id: nil)
      ...
      def create_case(identifier:, owner:)
      ...
      def update_case_metadata(case_id:, resource:, update_timestamp: false, update_owner: false)
      ...
      def find_account_by_ror(ror_id)
      ...
      def find_account_by_tenant(tenant)
